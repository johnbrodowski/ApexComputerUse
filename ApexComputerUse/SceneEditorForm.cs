...
namespace ApexComputerUse
{
    ...
    public partial class SceneEditorForm : Form
    {
        ...
        private void refreshTimer_Tick(object sender, EventArgs e)
        {
            ...
            // Skip heavy UI refresh while the user is actively dragging
            if (_dragMode is DragMode.Move or DragMode.Resize or DragMode.Place or DragMode.Rotate) return;
        ...
        private void btnToolTriangle_Click(object s, EventArgs e) => SetTool("triangle");
        ...
        private void canvasPanel_Paint(object sender, PaintEventArgs e)
        {
            ...
            // Draw selected shape on top (live during move/resize/rotate)
        ...
        {
            // While moving/resizing/rotating, exclude the shape from the cache so it doesn't ghost
            string? excludeId = (_dragMode == DragMode.Move || _dragMode == DragMode.Resize || _dragMode == DragMode.Rotate) ? _curShapeId : null;
        ...
        private void DrawSelectionBox(Graphics g, AIDrawingCommand.ShapeCommand s)
        {
            ...
            float rhr  = 5f / _scale;
            using var stemPen  = new Pen(Color.FromArgb(0, 230, 153), 1f / _scale);
            using var rhBrush  = new SolidBrush(Color.White);
            using var rhPen    = new Pen(Color.FromArgb(0, 230, 153), 1f / _scale);
            g.DrawLine(stemPen, rhx, rect.Top, rhx, rhy);
            g.FillEllipse(rhBrush, rhx - rhr, rhy - rhr, rhr * 2, rhr * 2);
        ...
        private void canvasPanel_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            if (_activeTool == "arrow")
            {
                ...
                {
                    // Rotation handle
                    var selForRotate = GetSelectedSceneShape()!;
                    var bbRot        = ShapeBBox(selForRotate.Shape)!.Value;
                    _rotateCenterX     = bbRot.X + bbRot.Width  / 2f;
                    _rotateCenterY     = bbRot.Y + bbRot.Height / 2f;
                    _dragMode          = DragMode.Rotate;
                    _dragStart         = sc;
                    _dragShapeSnapshot = CloneShape(selForRotate.Shape);
                    _cacheValid        = false;
        ...
        private void canvasPanel_MouseMove(object sender, MouseEventArgs e)
        {
            ...
            else if (_dragMode == DragMode.Rotate && _dragShapeSnapshot != null && _curShapeId != null)
            {
                var ss = GetSelectedSceneShape();
                if (ss == null) return;
                float startAng = MathF.Atan2(_dragStart.Y - _rotateCenterY, _dragStart.X - _rotateCenterX);
                float curAng   = MathF.Atan2(sc.Y          - _rotateCenterY, sc.X          - _rotateCenterX);
                float deltaDeg = (curAng - startAng) * 180f / MathF.PI;
                ss.Shape.Rotation = _dragShapeSnapshot.Rotation + deltaDeg;
                _cacheValid = false;
                canvasPanel.Invalidate();
            }
        ...
        private void canvasPanel_MouseUp(object sender, MouseEventArgs e)
        {
            ...
            if (_dragMode == DragMode.Rotate && _curShapeId != null)
            {
                var ss = GetSelectedSceneShape();
                if (ss != null)
                {
        ...
        private int HitTestHandle(PointF sc)
        {
            ...
            if (bbRot.HasValue)
            {
                var rr  = bbRot.Value;
                rr.Inflate(4, 4);
                float rhx = rr.Left + rr.Width / 2f;
                float rhy = rr.Top  - 24f / _scale;
                if (Dist(sc, new PointF(rhx, rhy)) <= tol) return 8;
            }
        ...
        private static Cursor HandleCursor(int handle) => handle switch
        {
            ...
            8      => Cursors.Hand,      // rotation handle
            _      => Cursors.SizeAll
        };
        ...
        protected override bool ProcessCmdKey(ref Message msg, Keys keyData)
        {
            switch (keyData)
            {
                ...
                case Keys.G: SetTool("triangle"); return true;
                case Keys.A: SetTool("arc");      return true;
            }
        ...
        private AIDrawingCommand.ShapeCommand MakeShape(string tool, PointF at)
        {
            ...
            return tool switch
            {
                ...
                "triangle" => new AIDrawingCommand.ShapeCommand { Type = "triangle", X = at.X, Y = at.Y, W = 80, H = 60, Color = hex, Fill = true },
                "arc"      => new AIDrawingCommand.ShapeCommand { Type = "arc",      X = at.X, Y = at.Y, W = 80, H = 80, Color = hex, StrokeWidth = 2 },
        ...
        private static AIDrawingCommand.ShapeCommand CloneShape(AIDrawingCommand.ShapeCommand s) =>
            new()
            {
                ...
                Rotation = s.Rotation, StartAngle = s.StartAngle, SweepAngle = s.SweepAngle
            };
