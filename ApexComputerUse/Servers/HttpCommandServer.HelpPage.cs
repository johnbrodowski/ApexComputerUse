    ...
    public partial class HttpCommandServer
    {
        ...
        [
            ...
            new("GET",  "/elements", "[type=ControlType] [id=mapId] [depth=n] [onscreen=true] [match=text] [collapseChains=true] [includePath=true] [properties=extra] [since=hash]",
                "Element tree for current window; returns treeHash for polling. When match= is supplied, depth= is ignored - the full tree is scanned to find matches (Name/AutomationId/Value/ClassName, case-insensitive substring)."),
            ...
            new("POST", "/exec",     "action=<action> [value=<input>] | actions=[{...},{...}] [stop_on_error=true]",
                "Execute an action on the current element. Pass actions=[...] for a batch: each entry is a full sub-request (cmd defaults to 'execute'); stop_on_error defaults to true. Errors carry an additive 'error_data' object with failed_pattern, supported_patterns, element_state, and a hint when applicable."),
            ...
            new("GET",  "/sysinfo",  null,
                "OS and machine info"),
            new("GET",  "/events",   "[types=<csv>] [windowId=<id>]",
                "Server-Sent Events stream of desktop changes (window-created, window-closed, window-title-changed). Long-lived; use 'curl -N'. Filters: types= for event types, windowId= for a specific stable window ID."),
            new("GET",  "/file",     "path=<absolute>",
                "Read a file as JSON. Gated by EnableFileIo + FileIoAllowedRoots config; refuses traversal, directories, and files >1 MB. Known text extensions return UTF-8 in 'content'; everything else returns base64."),
        ...
        [
            ...
            new("Wait", "wait",
            [
                new("waitfor",       null,                 "predicate=<equals|contains|not-empty|visible|gone> [property=<value|text|name|isvisible|isenabled>] [expected=<text>] [timeout=<ms>] [interval=<ms>]",
                    "Poll the current element until predicate satisfied or timeout. visible/gone are element-level (no property/expected needed). Success data includes elapsed_ms, property, predicate; timeout error_data includes last_observed."),
                new("wait-window",   null,                 "predicate=<equals|contains|not-empty|gone> expected=<title-substring> [timeout=<ms>] [interval=<ms>]",
                    "Poll desktop window list until a window title satisfies predicate. On match, sets the current window so the next /find or /elements works without a redundant window= field. Timeout error_data includes last_observed_titles."),
            ]),
